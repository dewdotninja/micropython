'''
lag3_pid_netpie_oop_multitask_v1_lost_connection.py
dew.ninja  Mar 2023

Update : Mar 2023

- when onboard button is pressed, disconnect NETPIE
- true multitasking version
...
- implement PID control with lag3 plant commanded from netpie
...
netpie_status = 1 # status of NETPIE connection
...
def disconnect_when_pressed():
    global netpie_status
    if not button.value(): # disconnect netpie when button pressed 
        ...
        while not button.value(): # blocked if switch pressed
            ...
            sleep_ms(3000)
    if not netpie_status: # reconnect
        netpie.init_client()
        print("NETPIE reconnected")
        netpie_status = 1
...
# ---- define a function to run PID on separate thread
def control_loop():
    global t_previous, y, u, t_data, data_idx, capture_flag
    while True:
...
try:
    while True:
            if online:
                # mimic communication loss
                disconnect_when_pressed()    
                ...
                if publish_delta>PUBLISH_PERIOD: # publish interval must be larger than PUBLISH_PERIOD
                    ...
                    sleep_ms(100) # just simple 100 ms delay
